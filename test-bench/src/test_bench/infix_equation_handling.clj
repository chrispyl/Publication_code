(ns test-bench.infix-equation-handling
		...
		(if ks 
			(let [params (:params ((first ks) m))
				  ...
				  values (map #(if (fileValues %) 
								((fileValues %) 0) ;if value is taken from filevalues, this is done here and only for the first value
								(:init-val (% m))) ;else it is calculated from the other kinds of equations
							  params)]
