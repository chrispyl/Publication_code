...
(defn create-subsystem-map-across-the-method [team-map system-map]
...
(defn euler-method [iteration step  function equation-atom param-names pull-from]
	(+ @(@equation-atom iteration) (* step (calc-new-value iteration function param-names pull-from))))		
...
(defn update-atom [iteration step param-names pull-from function equation-atom store-at]
	(swap! store-at assoc (+ iteration 1) (deliver (@store-at (+ iteration 1)) (euler-method iteration step function equation-atom param-names pull-from))))
	...
	(let  [simple-eqs-map (remove-elements true system-map)
		   ...
		   calculations (loop [iteration 0]
							(when-not (= iteration iterations)
								(doall
									(map #(update-atom iteration step % %2 %3 %4 %5)
										dif-eqs-param-names
										diff-eqs-pull
										dif-eqs-functions
										dif-eqs-atoms
...
(defn across-the-method-integration [start end step subsystems system-map fileValues]
	(let [iterations (long (quot (- end start) step))
	     fns-atoms-map (create-fns-atoms-map system-map iterations fileValues)
		 gos (doall 
				(map #(async/go (calc-funcs-in-chunk iterations step fns-atoms-map %)) subsystems))]
		fns-atoms-map))	
