...
;teams - list of lists. Each sublist has as first element the reachability of the team, as a set
...
(defn find-independent-teams [teams in-map]
	(let [independent (filter      
						(fn [team](every? true? (map #(clset/subset? (in-map %) (first team)) (drop 1 team)))) ;drop because the first element is the reachability of the team
							teams)]
		independent))				
...
;system-map - map							   
(defn create-subsystem-map [teams system-map available-cores]
